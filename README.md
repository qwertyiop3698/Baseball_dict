...
이 프로젝트는 정적 HTML만 여는 방식이 아니라 Flask 서버로 실행해야 합니다.
...
VS Code Live Server처럼 `http://127.0.0.1:5500/...` 주소로 열면 `/ask` API가 동작하지 않아 검색 기능을 사용할 수 없습니다.
...
`app.py`는 `.env` 파일 또는 환경 변수에서 `API_KEY` 값을 읽어 Gemini API 호출에 사용합니다.
